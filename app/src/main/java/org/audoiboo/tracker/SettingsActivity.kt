...
object AppPrefs {
    ...
    fun baseFolder(context: Context): String =
        prefs(context).getString(BASE, "Audoiboo")?.trim()?.ifBlank { "Audoiboo" } ?: "Audoiboo"
...
private fun SettingsScreen(activity: ComponentActivity) {
    ...
    fun save() {
        AppPrefs.save(
            activity, base, author, dev, dark, ask, wifi, unpack,
            updateWifi, showPage, showFind, autoArchives
        )
    }

    fun saveAutomatic() {
        BackupStore.setAutomaticSettings(activity, autoSettings, autoBookmarks, autoStatistics)
        BackupStore.setAutomaticBackupPath(activity, autoPath)
        BackupStore.maybeCreateDailyBackup(activity)
    }
        ...
        if (uri != null) {
            runCatching {
                activity.contentResolver.openOutputStream(uri)?.bufferedWriter()?.use {
                    it.write(BackupStore.exportJson(activity))
                }
            }.onSuccess {
                Toast.makeText(activity, "Резервну копію створено", Toast.LENGTH_SHORT).show()
            }.onFailure {
                Toast.makeText(activity, "Помилка резервної копії: ${it.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    val restoreLauncher = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        if (uri != null) {
            runCatching {
                ...
                Toast.makeText(activity, "Помилка відновлення: ${it.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
            ...
            Column(
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Text("Налаштування вбудованого плеєра", style = MaterialTheme.typography.titleMedium)
                        Text("Сам плеєр доступний з головного екрана.", style = MaterialTheme.typography.bodySmall)
                        OutlinedButton(
                            onClick = { activity.startActivity(Intent(activity, PlayerSettingsActivity::class.java)) },
                            modifier = Modifier.fillMaxWidth()
                        ) { Text("Налаштування плеєра") }
                    }
                }
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        SettingRow("Розпаковувати ZIP", "Кожна книга — у власну папку") {
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        OutlinedTextField(
                            value = autoPath,
                            onValueChange = { autoPath = it },
                            label = { Text("Папка автобекапів") },
                            supportingText = { Text("За замовчуванням: ${BackupStore.DEFAULT_AUTO_PATH}") },
                            modifier = Modifier.fillMaxWidth(),
                            singleLine = true
                        )
                        Button(onClick = {
                            BackupStore.setAutomaticBackupPath(activity, autoPath)
                            Toast.makeText(activity, "Шлях автобекапів збережено", Toast.LENGTH_SHORT).show()
                        }) { Text("Зберегти шлях") }
                    }
                }
                SettingCard(
                    "Автоматичне резервне копіювання налаштувань",
                    "Щоденна локальна резервна копія налаштувань застосунку й плеєра"
                ) {
                    Switch(checked = autoSettings, onCheckedChange = { autoSettings = it; saveAutomatic() })
                }
                SettingCard(
                    "Автоматичне резервне копіювання закладок",
                    "Щоденна резервна копія закладок плеєра"
                ) {
                    Switch(checked = autoBookmarks, onCheckedChange = { autoBookmarks = it; saveAutomatic() })
                }
                SettingCard(
                    "Автоматичне резервне копіювання статистики",
                    "Щоденна резервна копія позицій і прогресу прослуховування"
                ) {
                    Switch(checked = autoStatistics, onCheckedChange = { autoStatistics = it; saveAutomatic() })
                }
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        Text("Повна ручна копія містить серії, книги, статуси, архіви, завантаження, налаштування плеєра, закладки та позиції відтворення.")
                        Button(
                            onClick = { backupLauncher.launch("Audoiboo-Tracker-backup.json") },
                            modifier = Modifier.fillMaxWidth()
                        ) { Text("Створити резервну копію") }
                        OutlinedButton(
                            onClick = { restoreLauncher.launch(arrayOf("application/json", "text/plain")) },
                            modifier = Modifier.fillMaxWidth()
                        ) { Text("Відновити з копії") }
                    }
                }
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp)) {
                        Text("Про додаток")
                        Text("Audoiboo Tracker dev", style = MaterialTheme.typography.bodySmall)
                    }
                }
