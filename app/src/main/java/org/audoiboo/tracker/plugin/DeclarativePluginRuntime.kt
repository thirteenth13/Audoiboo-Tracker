...
class DeclarativePluginRuntime(private val sandbox:PluginSandbox,private val decoder:DeclarativeEntrypointDecoder=JsonDeclarativeEntrypointDecoder){
 ...
 private fun collectLis10BookCards(document:Element,manifest:PluginPackageManifest,canonical:CanonicalSeriesMatchInput,author:String,output:MutableList<SourceBookRef>){document.select("a.mcard[href*='/audio/']").forEachIndexed{index,card->val cardAuthor=card.selectFirst(".mcard-a")?.text()?.trim().orEmpty();if(cardAuthor.isNotBlank()&&!sameAuthor(cardAuthor,author))return@forEachIndexed;val href=card.attr("href").takeIf{it.isNotBlank()}?:return@forEachIndexed;val bookUrl=canonicalPluginBookUrl(manifest.id,resolveUrl(card,href))?:return@forEachIndexed;val title=card.selectFirst(".mcard-t")?.text()?.trim()?.takeIf{it.isNotBlank()}?:return@forEachIndexed;val match=SourceIdentityMatcher.bestBookMatch(SourceBook(sourceId=manifest.id,url=bookUrl,title=title,authors=listOf(SourceAuthor(author)),seriesTitle=canonical.title),canonical.books)?.takeIf{it.disposition==MatchDisposition.AUTO_ACCEPT};output+=SourceBookRef(url=bookUrl,title=title,number=match?.value?.number?:((index+1).toDouble()))}}
