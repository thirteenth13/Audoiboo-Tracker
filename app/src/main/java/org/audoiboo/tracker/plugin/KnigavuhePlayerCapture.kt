    ...
    fun capture(manifest: PluginPackageManifest, rule: PluginMediaCaptureRule, pageUrl: String, onComplete: (PluginMediaCaptureResult) -> Unit) {
        Handler(Looper.getMainLooper()).post {
            ...
            fun remember(raw: String?, source: String) {
                ...
                synchronized(found){if(keys.add(key)&&found.size<rule.maxResults){found+=url;if(diagnostics.size<180)diagnostics+="accepted-$source:${url.take(500)}"}}
            }
            ...
            // The declarative rule originally did two preparation steps. CI 850 proved that only
            // toggling "Большие отрезки" is insufficient: the long-track rows never enter the DOM.
            // First activate "Слушать полностью", then toggle long segments, then arm capture.
            fun fullPlayerScript():String="""
                (()=>{
                  const n=s=>String(s||'').replace(/\s+/g,' ').trim();
                  let a=[...document.querySelectorAll('body *')].filter(e=>/слушать полностью/i.test(n(e.innerText||e.textContent)));
                  a=a.filter(e=>![...e.children].some(c=>/слушать полностью/i.test(n(c.innerText||c.textContent))));
                  a=a.filter(e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>3&&r.height>3&&s.display!=='none'&&s.visibility!=='hidden'});
                  if(!a.length){AudoibooKvCapture.event('full-player-miss');return null}
                  let c=a[0].closest('button,a,[role=button],[onclick]')||a[0];c.scrollIntoView({block:'center'});const r=c.getBoundingClientRect();
                  AudoibooKvCapture.event('full-player-hit:'+c.tagName+':'+String(c.className||'').slice(0,70));return{x:r.left+r.width/2,y:r.top+r.height/2}
                })()
            """.trimIndent()

            fun toggleLongScript():String="""
                (()=>{
                  const n=s=>String(s||'').replace(/\s+/g,' ').trim(),all=[...document.querySelectorAll('body *')];
                  const t=all.find(e=>/большие отрезки/i.test(n(e.innerText||e.textContent)));
                  if(!t){AudoibooKvCapture.event('toggle-long=miss');return null}
                  let box=t.matches?.('input[type=checkbox]')?t:(t.querySelector?.('input[type=checkbox]')||t.parentElement?.querySelector?.('input[type=checkbox]'));
                  if(box){if(box.checked){AudoibooKvCapture.event('toggle-long=ready');return null}box.scrollIntoView({block:'center'});const r=box.getBoundingClientRect();AudoibooKvCapture.event('toggle-long=box');return{x:r.left+r.width/2,y:r.top+r.height/2}}
                  let c=t.closest('button,a,label,[role=button],[onclick]')||t;c.scrollIntoView({block:'center'});const r=c.getBoundingClientRect();AudoibooKvCapture.event('toggle-long=label');return{x:r.left+r.width/2,y:r.top+r.height/2}
                })()
            """.trimIndent()

            fun rowScript(index:Int):String="""(()=>{const idx=$index,n=s=>String(s||'').replace(/\s+/g,' ').trim();const match=t=>{const m=n(t).match(/_(\d+)(?:\s+\d{1,2}:\d{2}(?::\d{2})?)?$/);return !!m&&Number(m[1])===idx};let a=[...document.querySelectorAll('body *')].filter(e=>match(e.innerText||e.textContent));a=a.filter(e=>![...e.children].some(c=>match(c.innerText||c.textContent)));a=a.filter(e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>3&&r.height>3&&r.height<220&&s.display!=='none'&&s.visibility!=='hidden'});a.sort((x,y)=>{const A=x.getBoundingClientRect(),B=y.getBoundingClientRect();return A.width*A.height-B.width*B.height});if(!a.length){AudoibooKvCapture.event('row-miss:'+idx);return null}const e=a[0],c=e.closest('button,a,[role=button],[onclick],li,[class*=track],[class*=item]')||e;c.scrollIntoView({block:'center'});const r=c.getBoundingClientRect();AudoibooKvCapture.event('row:'+idx+':'+n(e.innerText||e.textContent).slice(0,100));return{x:r.left+Math.min(Math.max(18,r.width*.12),r.width/2),y:r.top+r.height/2}})()"""
            fun traverse(index:Int=0,misses:Int=0,waits:Int=0){if(finished.get())return;if(index>=90||misses>=4){diagnostics+="kv-track-stop:index=$index misses=$misses";handler.postDelayed({scan();finish("kv-complete")},900);return};webView.evaluateJavascript(rowScript(index)){raw->val p=rect(raw);if(p==null){if(index==0&&waits<16){diagnostics+="kv-row-wait=${waits+1}";handler.postDelayed({traverse(0,0,waits+1)},500)}else handler.postDelayed({traverse(index+1,misses+1,waits)},150)}else{tap(p.first,p.second);handler.postDelayed({scan();traverse(index+1,0,waits)},420)}}}

            fun armAndTraverse(){synchronized(found){found.clear();keys.clear()};armed.set(true);diagnostics+="kv-armed-after-prepare";scan();traverse()}
            fun preparePlayer(){
                webView.evaluateJavascript(fullPlayerScript()){rawFull->
                    val full=rect(rawFull)
                    if(full!=null){tap(full.first,full.second);diagnostics+="kv-full-player-native"}else diagnostics+="kv-full-player-skip"
                    handler.postDelayed({
                        webView.evaluateJavascript(toggleLongScript()){rawToggle->
                            val toggle=rect(rawToggle)
                            if(toggle!=null){tap(toggle.first,toggle.second);diagnostics+="kv-toggle-long-native"}else diagnostics+="kv-toggle-long-skip"
                            handler.postDelayed({armAndTraverse()},1800)
                        }
                    },1200)
                }
            }
            ...
            webView.addJavascriptInterface(object{@JavascriptInterface fun media(value:String?)=handler.post{remember(value,"js")};@JavascriptInterface fun event(value:String?)=handler.post{if(!value.isNullOrBlank()&&diagnostics.size<220)diagnostics+="js:$value"}},BRIDGE)
            webView.webViewClient=object:WebViewClient(){
                ...
                override fun onPageFinished(view:WebView,url:String){if(!PluginWebViewMediaCaptureRuntime.isAllowedPage(manifest,rule,url)||finished.get())return;diagnostics+="kv-loaded";installHooks();handler.postDelayed({preparePlayer()},900)}
            }
            handler.postDelayed({finish("kv-timeout")},minOf(rule.timeoutMs+10000,35000));webView.loadUrl(pageUrl)
        }
    }
}
