...
class Lis10BookWebViewMediaCapture(private val context: Context) {
 ...
 fun capture(pageUrl:String,timeoutMs:Long=24_000L,onComplete:(Result)->Unit){require(isAllowedPage(pageUrl));Handler(Looper.getMainLooper()).post{
  ...
  fun remember(raw:String?){if(!armed.get())return;val u=normalizeMediaUrl(pageUrl,raw)?:return;val k=mediaKey(u)?:return;synchronized(found){if(found.size<MAX_MEDIA_URLS&&keys.add(k))found+=u}}
 ...
 companion object{
  ...
  fun normalizeMediaUrl(pageUrl:String,raw:String?):String?=runCatching{val value=raw?.trim()?.replace("&amp;","&").orEmpty();if(value.isBlank())return@runCatching null;URI(pageUrl).resolve(value).toString().takeIf(::isAudioUrl)}.getOrNull()
