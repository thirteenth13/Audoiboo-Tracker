...
class PoleknigWebViewMediaCapture(private val context: Context) {
        ...
        Handler(Looper.getMainLooper()).post {
            ...
            webView.webViewClient = object : WebViewClient() {
                ...
                override fun onPageFinished(view: WebView, url: String) {
                    ...
                    listOf(800L, 2_500L, 5_000L, 8_000L).forEach { delay ->
                        handler.postDelayed({ if (!finished.get()) view.evaluateJavascript(ACTIVATE_AND_SCAN, null) }, delay)
                    }
                    handler.postDelayed({ if (!finished.get() && snapshot().isNotEmpty()) finish("captured") }, 12_000L)
                }
            }
    ...
    companion object {
        ...
        private val INSTALL_HOOKS = """
            (() => {
              if (window.__audoibooPoleHooks) return 'already';
              window.__audoibooPoleHooks = true;
              const emit = u => { try { if (u) AudoibooPoleCapture.media(String(u)); } catch (_) {} };
        ...
        private val ACTIVATE_AND_SCAN = """
            (() => {
              ...
              (html.match(/(?:https?:\/\/[^"'<>\\s]+)?\/files\/\d+(?:\?[^"'<>\\s]*)?/gi) || [])
                .forEach(u => emit(new URL(u, location.href).href));
              ...
              AudoibooPoleCapture.event('clicks='+likely.length);
